...
export function ensurePortalSchema(): Promise<void> {
  ensured ??= (async () => {
    ...
    const stmts = [
      ...
      `alter table dealerships add column if not exists crm_onboard_url text not null default ''`,
      ...
      `alter table vehicles add column if not exists tile_rev integer not null default 1`,
