...
test("dealer control centre has no inventory link and assigns the signed-in employee", () => {
  ...
  assert.match(partner, /deleteDeskDeal/);
  assert.match(access, /user: DealerUser/);
  assert.match(schema, /dealer_users/);
  assert.match(schema, /crm_onboard_url/);
  assert.match(neu, /Save quote/);
  assert.match(neu, /First name/);
  assert.match(neu, /Last name/);
  assert.match(neu, /inputMode="decimal"/);
  assert.doesNotMatch(neu, /type="number"/);
  assert.match(people, /Add person/);
  assert.match(people, /Delete/);
  assert.match(shell, /Team/);
  const stages = readFileSync(new URL("../src/lib/desk/stages.ts", import.meta.url), "utf8");
  assert.match(stages, /quoted/);
  assert.match(stages, /compliance/);
  const pill = readFileSync(new URL("../src/components/desk/stage-pill.tsx", import.meta.url), "utf8");
  assert.match(pill, /StagePill/);
  const hero = readFileSync(new URL("../src/lib/desk/hero.ts", import.meta.url), "utf8");
